...
class Conv2d_Q(nn.Conv2d):
    ...
    def forward(self, input, order=None):
        ...
        conv_out = F.conv2d(input, weight_q, bias_q, self.stride, self.padding, self.dilation, self.groups)
        input_dict.append(self.quantize_out(input, False))
        output_dict.append(self.quantize_out(conv_out, False))
        return self.quantize_out(conv_out)

    def quantize_out(self, x, div=True):
        a_bit = 7
        scale = 2 ** a_bit
        x = x * scale
        x = x.clamp(min=-32768, max=32767)
        x = torch.floor(x) / scale if div else torch.floor(x)
        return x
